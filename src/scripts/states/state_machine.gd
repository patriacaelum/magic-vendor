...
signal state_changed(state: State.STATENAME)
...
func _ready() -> void:
    ...
    for child: Node in self.get_children():
        ...
        child.property_set.connect(self._on_state_property_set)
...
func _on_state_finished() -> void:
    ...
    self.state_changed.emit(self._tda[-1])
